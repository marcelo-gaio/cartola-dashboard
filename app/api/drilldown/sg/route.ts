...
export async function GET(req: Request) {
  ...
  // posições aceitas: GOL/LAT/ZAG/TOT
  const posId = pos === "TOT" ? null : (POS_TO_ID[pos] ?? null);
  if (pos !== "TOT" && !(posId === 1 || posId === 2 || posId === 3)) {
    return NextResponse.json({ ok: false, error: "invalid pos (use GOL|LAT|ZAG|TOT)" }, { status: 400 });
  }

  let q = sb
    .from("picks")
    .select("atleta_name, club_id, points, had_sg, position_id, is_home, team_rounds(round)")
    .eq("team_id", teamId)
    .in("position_id", [1, 2, 3]);
  ...
  // lookup clubes (pegamos * para não quebrar se o nome do campo de escudo variar)
